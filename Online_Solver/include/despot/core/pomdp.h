...
public:
	...
	virtual bool Step(State& state, double randomNums, int action, double& reward, OBS_TYPE& obs) const = 0;
	 ...
	 */
	virtual bool Step(State& state, int action, double& reward, OBS_TYPE& obs) const;
	 ...
	 * Display
	 * ========================================================================*/
	virtual void DisplayParameters(std::ofstream & out) const {};
	/**
	 * Prints a state.
	 */
	 ...
	 * Allocate a state.
	 */
	virtual State* Allocate(STATE_TYPE state_id = s_DEFAULT_CTOR_STATE, double weight = 0) const = 0;
