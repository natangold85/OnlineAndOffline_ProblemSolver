...
void POMCP::UpdateHistory(int action, OBS_TYPE obs)
{
	history_.Add(action, obs);
	prior_->Add(action, obs);
	belief_->Update(action, obs);
...
{
	OnlineSolverModel::SendTree(state, root_);
}
...
void POMCP::GetTreeProperties(Tree_Properties & treeProp) const // NATAN CHANGES
{
	...
	for (int a = 0; a < numChildren; ++a)
	{
		...
		if (actionTree != nullptr)
		{
			...
			treeProp.m_actionsChildren[a].m_height = actionTree->height();

			maxHeight = max(actionTree->height(), maxHeight);
		...
		else
		{
			...
			treeProp.m_actionsChildren[a].m_height = 0;
		}
	}
...
void POMCP::GetSingleActionTreeProp(SingleNodeTreeProp & treeProp, int action) const
{
	...
	if (actionTree != nullptr)
	{
		...
		treeProp.m_height = actionTree->height();
	}
	else
	{
...
VNode* POMCP::CreateVNode(int depth, const State* state, POMCPPrior* prior,
	...
	OnlineSolverModel::ChoosePreferredAction(prior, model, rewardsVec);
...
double POMCP::Simulate(State* particle, RandomStreams& streams, VNode* vnode,
	...
	double reward;
	OBS_TYPE obs;
	bool terminal = model->Step(*particle, streams.Entry(particle->scenario_id), action, reward, obs);
...
double POMCP::Rollout(State* particle, RandomStreams& streams, int depth,
	...
	double reward;
	OBS_TYPE obs;
	bool terminal = model->Step(*particle, streams.Entry(particle->scenario_id), reward, obs);
	if (!terminal) {
		prior->Add(action, obs);
		streams.Advance();
		reward += Globals::Discount()
...
double POMCP::Rollout(State* particle, int depth, const DSPOMDP* model,
	...
	int action = OnlineSolverModel::ChoosePreferredAction(prior, model, offlineReward);
	...
	// if reward was set by step return reward else if offline reward is valid return it o.w. return 0
	return reward != 0 ? reward : offlineReward != -100.0 ? offlineReward : 0.0; // NATAN CHANGES
}
...
ValuedAction POMCP::Evaluate(VNode* root, vector<State*>& particles,
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		while (!streams.Exhausted()) {
			...
			double reward;
			OBS_TYPE obs;
			bool terminal = model->Step(*copy, streams.Entry(copy->scenario_id), reward, obs);
