...
public:
	...
	virtual void AttackOnline(locationVec & objLocations, int objIdx, int targetIdx, const locationVec & shelters, int gridSize, double random) const override;
	/// calculation of attack for offline solver given locations, gridSize. attack result are update in result 
	virtual void AttackOffline(const locationVec & objLocations, int objIdx, int targetIdx, const locationVec & shelters, int gridSize, Attack::shootOutcomes & result) const;
