...
VNode* POMCP::CreateVNode(int depth, const State* state, POMCPPrior* prior,
	...
	nxnGrid::ChoosePreferredAction(state, prior, model, rewardsVec);
...
double POMCP::Rollout(State* particle, int depth, const DSPOMDP* model,
	...
	int action = nxnGrid::ChoosePreferredAction(particle, prior, model, offlineReward);//NATAN CHANGES SOLVER
