...
bool Evaluator::RunStep(int step, int round) {
	...
	{
		double reward;
		action = static_cast<nxnGrid *>(model_)->ChoosePreferredAction(state_, static_cast<POMCP *>(solver_)->GetPrior(), model_, reward);
	}

	//std::cout << "insert action wanted:\n";
	//std::cin >> action;
	...
	*out_ << "-----------------------------------Round " << round
	...
	if (terminal) {
		...
		// win = 1, loss = 2, tie = 0
		int result = reward == nxnGrid::REWARD_WIN ? 1 : nxnGrid::REWARD_LOSS ? 2 : 0;
