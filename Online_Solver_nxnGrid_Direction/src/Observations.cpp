...
int Observation::NonObservedLocation(int gridSize)
{
...
double ObservationByDistance::GetProbObservation(int selfLoc, int objLoc, int gridSize, int observation) const
{
	...
	if ((observation != objLoc) & (observation != NonObservedLocation(gridSize)))
		return 0.0;
	...
	return pSuccess * (observation == objLoc) + (1 - pSuccess) * (observation == NonObservedLocation(gridSize));
}
...
int ObservationByDistance::GetObservationObject(int selfLoc, int objLoc, int gridSize, double randomNum) const
{
	...
	if (randomNum < 0)
		return NonObservedLocation(gridSize);
}

void ObservationByDistance::InitObsAvailableLocations(int selfLoc, int objLoc, const locationVec & state, int gridSize, locationVec & observableLocations) const
{
	...
	if (objLoc != gridSize * gridSize)
		observableLocations.emplace_back(gridSize * gridSize + 1);
}
