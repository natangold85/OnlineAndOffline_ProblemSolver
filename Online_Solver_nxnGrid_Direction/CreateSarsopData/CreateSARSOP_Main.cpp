...
void RunSolver(std::string &fname, std::string &policy_name, std::string &solverOutF)
{
	...
	if (stat != 0)
		ErrorAndExit("running SARSOP solver");
}
...
void RunSimulator(std::string &pomdpFName, std::string &policy_name)
{
	...
	if (stat != 0)
		ErrorAndExit("running SARSOP simulator");
}
...
void CreateLUT(nxnGridOffline * model, std::string & prefix, lutSarsop & sarsopMap)
{
	...
	std::ofstream pomdpFile(pomdpFName, std::ios::out);
	if (pomdpFile.fail())
		ErrorAndExit("open pomdp file");

	// write pomdp file
	model->SaveInPomdpFormat(pomdpFile);
	pomdpFile.close();
